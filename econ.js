...
(function () {
  ...
  function renderForm() {
    ...
    placeholder.value = '';
    placeholder.textContent = fields.state.placeholder;
    placeholder.disabled = true;
    placeholder.selected = true;
    select.appendChild(placeholder);
    content.states.forEach(s => {
  ...
  function validateForm() {
    ...
    const checks = {
      ...
      current_rent: v => stripCurrency(v) >= 0,
      years_experience: v => { const n = parseInt(v); return n >= 0 && n <= 60; },
  ...
  async function handleSubmit(e) {
    ...
    formValues = {
      ...
      years_experience: parseInt(document.getElementById('years_experience').value.trim()),
  ...
  function populateResults() {
    ...
    // Edge case: minimal or negative gap
    if (cumulative <= 1000) {
      ...
      // Show systemic stat as secondary
      if (worth.worthGap) {
