...
(function () {
  ...
  function bindEvents() {
    ...
    function updateFormProgress() {
      // Progress bar removed — function retained as a no-op
      // because saveFormProgress/loadFormProgress still call it.
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      ['results-card', 'value-generated', 'wages-received', 'hero-stat',
       'hero-context', 'secondary-stat', 'survival-metrics', 'opportunity-cost', 'primary-cta'].forEach(function(id) {
        var el = document.getElementById(id);
      ...
      announce('Results calculated. Your productivity gap is ' + formatCurrency(resultsData.impact.summary.unrealized_productivity_gains) + '.');
      ...
      // Survival metrics reveal (earlier for elevated emphasis)
      setTimeout(function() {
  ...
  // ------------------------------------
  function calculateSurvivalMetrics(gapAnnual, annualSalary) {
    var dailyWage = annualSalary / 260; // ~260 working days per year
    var daysWorkedFree = dailyWage > 0 ? Math.round(gapAnnual / dailyWage) : 0;

    // Derive months directly from working days for intuitive consistency
    // ~21.67 working days per month (260 / 12)
    var workingDaysPerMonth = 260 / 12;
    var rentMonths = daysWorkedFree > 0 ? parseFloat((daysWorkedFree / workingDaysPerMonth).toFixed(1)) : 0;
  ...
  function populateSurvivalMetrics() {
    ...
    var metrics = calculateSurvivalMetrics(annualGap, resultsData.annualSalary);
  ...
  function populateOpportunityCost() {
    ...
    if (!worth || !worth.opportunityCost) {
    ...
    document.getElementById('opp-cost-daily').textContent = formatCurrency(annualGap / 260);
    document.getElementById('opp-cost-weekly').textContent = formatCurrency(annualGap / 52);
    document.getElementById('opp-cost-monthly').textContent = formatCurrency(annualGap / 12);
  ...
  function populateResults() {
    ...
    if (cumulative <= 0) {
      // Worker is fairly compensated or overpaid relative to industry value
  ...
  function populateBreakdown() {
    ...
    const rows = [
      ...
      { label: sections.worth_gap, value: worth.worthGap ? formatCurrency(worth.worthGap.annual) + '/yr' : 'N/A', note: worth.marketData ? 'Market median: ' + formatCurrency(worth.marketData.adjustedMedian * 1680) + '/yr (' + worth.marketData.source + ')' : '' },
  ...
  function populateGapDecomposition() {
    ...
    var segments = [
      { key: 'depreciation', color: 'var(--gold-primary, #D4A054)', data: decomp.depreciation },
      { key: 'taxes', color: 'var(--text-muted, #8a8a8a)', data: decomp.taxes },
      { key: 'netProfit', color: 'var(--red-primary, #C45B4A)', data: decomp.netProfit }
    ];
  ...
  function populateCompensationContext() {
    ...
    contextBlock.className = 'formula-block';
    ...
    if (parts.length > 0) {
      ...
      ctxLabel.className = 'formula-label';
      ctxLabel.textContent = 'Your Compensation Context';
      contextBlock.appendChild(ctxLabel);

      parts.forEach(function(text) {
        var p = document.createElement('div');
        p.className = 'assumption-item';
        p.textContent = text;
        contextBlock.appendChild(p);
      });
      inner.appendChild(contextBlock);
    }
    ...
    if (provenance && provenance.length > 0) {
      ...
      provBlock.className = 'formula-block';
      var provLabel = document.createElement('div');
      provLabel.className = 'formula-label';
      provLabel.textContent = 'Data Sources & Vintage';
      ...
      provenance.forEach(function(p) {
        ...
        if (p.url) {
          ...
          item.appendChild(document.createTextNode(' \u2014 ' + p.agency + (p.vintage ? ' (' + p.vintage + ')' : '')));
        } else {
          item.textContent = p.name + ' \u2014 ' + p.agency + (p.vintage ? ' (' + p.vintage + ')' : '');
        }
        provBlock.appendChild(item);
      });
      inner.appendChild(provBlock);
    }
  }
  ...
  function populateMethodology() {
    ...
    m.sources.forEach(function(s) {
      if (s.url) {
        ...
        note.className = 'source-link source-note';
        note.textContent = s.name + ' \u2014 ' + s.type;
  ...
  // ------------------------------------
  // VIDEO PREVIEW — phone mockup with live data
  // ------------------------------------
  function populateVideoPreview() {
    ...
    document.getElementById('vp-gap').textContent = formatCurrency(cumulative);
    document.getElementById('vp-context').textContent = content.results.hero_context_template;
    document.getElementById('vp-annual').textContent = formatCurrency(annualGap) + '/yr';

    var sm = resultsData.survivalMetrics;
    if (sm && sm.daysWorkedFree > 0) {
      document.getElementById('vp-days').textContent = sm.daysWorkedFree;
      document.getElementById('vp-months').textContent = sm.rentMonths;
      document.getElementById('vp-metrics').hidden = false;
    } else {
      document.getElementById('vp-metrics').hidden = true;
    }
    ...
    if (window.RupturaVideoCard && window.RupturaVideoCard.isSupported()) {
      ...
      window.RupturaVideoCard.generate(resultsData, content,
        function(p) {
          ...
          var currentMsg = msgIndex < progressMessages.length ? progressMessages[msgIndex] : 'Finishing up';
          textEl.textContent = currentMsg + '... ' + pct + '%';
        },
        function() {
    ...
    try {
      ...
      const data = await fetchJSON('/api/negotiation-script', {
        ...
        body: {
          ...
          frequency: 'annual',
          market_median: worth.marketData ? worth.marketData.adjustedMedian * 1680 : null,
